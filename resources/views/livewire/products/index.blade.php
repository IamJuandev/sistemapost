        ...
        <div x-data="{}" x-show="$wire.showCreateForm || $wire.showEditForm" x-transition
            ...
            <div class="p-6 bg-slate-50 dark:bg-slate-900/70">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-6">
                        <x-form.input name="name" label="Nombre" wire:model="name" required />
                        <x-form.select name="category_id" label="Categoría"
                            :options="$categories->pluck('name', 'id')->toArray()" wire:model="category_id" required
                            :placeholder="'Seleccionar...'" />
                        <x-form.select name="supplier_id" label="Proveedor"
                            :options="$suppliers->pluck('company_name', 'id')->toArray()" wire:model="supplier_id"
                            required :placeholder="'Seleccionar...'" />
                        <x-form.input name="barcode" label="Código de Barras" wire:model="barcode" required />
                    </div>
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            <x-form.input name="cost_price" label="Precio de Costo" type="number" step="0.01"
                                wire:model.live="cost_price" required />
                            <x-form.input name="profit_margin" label="Margen (%)" type="number" step="0.01"
                                wire:model.live="profit_margin" required />
                        </div>
                        <x-form.input name="tax_rate" label="Impuesto (%)" type="number" step="0.01"
                            wire:model.live="tax_rate" required />
                        <x-form.input name="selling_price" label="Precio Venta (Calculado)" wire:model="selling_price"
                            readonly disabled />
                        <x-form.input name="stock" label="Stock Inicial" type="number" wire:model="stock" required />
                    </div>
                </div>

                <div class="mt-6 flex items-center space-x-3">
                    <x-form.button type="button" wire:click="{{ $showEditForm ? 'update' : 'store' }}"
                        variant="primary">
                        {{ $showEditForm ? 'Actualizar Producto' : 'Guardar Producto' }}
                    </x-form.button>
                    <x-form.button type="button" wire:click="cancel" variant="secondary">
                        Cancelar
                    </x-form.button>
                </div>
            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-12 w-12">
                                    ...
                                    <img class="h-12 w-12 rounded-md object-cover" src="{{ $product->image_url }}"
                                        alt="Imagen de {{ $product->name }}">
                                    @else
                                    <div
                                        class="flex items-center justify-center h-full w-full bg-slate-100 dark:bg-slate-700 rounded-md">
                                        <span class="text-lg font-bold text-slate-500 dark:text-slate-300">
                                            {{-- Toma las primeras 2 letras del nombre del producto --}}
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-slate-500 dark:text-slate-400">{{ $product->category->name
                                        ?? 'Sin categoría' }}</div>
                                    <div class="text-xs text-slate-600 dark:text-slate-400">Código:
                                        {{$product->barcode}}</div>
                                </div>
                            </div>
                        </td>
