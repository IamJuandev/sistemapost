<div class="bg-slate-50 dark:bg-slate-900 min-h-screen">
    ...
    <div class="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
                <h1 class="text-3xl font-bold text-slate-800 dark:text-slate-100">Detalle de Factura de Venta</h1>
                <p class="mt-1 text-slate-500 dark:text-slate-300">Revisa la información de la venta registrada.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
            ...
            <div
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase mb-2">Cliente</p>
                    <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">{{ $invoice->customer->first_name
                        }} {{ $invoice->customer->last_name }}</h3>
                    <p class="text-slate-600 dark:text-slate-300">{{ $invoice->customer->phone ?? 'N/A' }}</p>
                    <p class="text-slate-600 dark:text-slate-300">{{ $invoice->customer->address ?? 'N/A' }}</p>
                </div>
                <div class="space-y-4">
                    <div class="grid grid-cols-2">
                        <span class="font-semibold text-slate-600 dark:text-slate-300">Fecha de Factura:</span>
                        <span class="text-slate-800 dark:text-slate-100 text-right">{{ $invoice->sale_date ?
                            $invoice->sale_date->format('d/m/Y') : 'N/A' }}</span>
                    </div>
                    <div class="grid grid-cols-2">
                    ...
                    <div class="grid grid-cols-2">
                        <span class="font-semibold text-slate-600 dark:text-slate-300">Método de Pago:</span>
                        <span class="text-slate-800 dark:text-slate-100 text-right">{{ $invoice->payment_method ??
                            'N/A' }}</span>
                    </div>
            ...
            <div class="p-6 md:p-8">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($invoice->saleItems as $item)
                        <tr>
                            ...
                            <td class="py-4 text-slate-600 dark:text-slate-300 text-right">${{
                                number_format($item->selling_price ?? 0, 2) }}</td>
                            <td class="py-4 font-semibold text-slate-800 dark:text-slate-100 text-right">${{
                                number_format(($item->selling_price * $item->quantity) ?? 0, 2) }}</td>
                        </tr>
                        @empty
                        <tr>
            ...
            <div
                ...
                <div class="w-full md:w-1/2 md:ml-auto space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-slate-800 dark:text-slate-100">${{
                            number_format($invoice->discount_amount ?? 0, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
